...
type StorageClassManager interface {
	List(ctx context.Context, id uint) ([]dto.StorageClass, error)
...
// List 列表
func (s *storageClassManager) List(ctx context.Context, id uint) ([]dto.StorageClass, error) {
	items, err := s.storageClass.List(ctx, id)
	if err != nil {
		return nil, err
	}
	return dto.ToStorageClassDTOs(items), nil
}

// Get 查询
func (s *storageClassManager) Get(ctx context.Context, id uint, name string) (*storagev1.StorageClass, error) {
	return s.storageClass.Get(ctx, id, name)
}

// Create 创建
func (s *storageClassManager) Create(ctx context.Context, id uint, body *storagev1.StorageClass) (*storagev1.StorageClass, error) {
	return s.storageClass.Create(ctx, id, body)
}

// Update 更新
func (s *storageClassManager) Update(ctx context.Context, id uint, body *storagev1.StorageClass) (*storagev1.StorageClass, error) {
	return s.storageClass.Update(ctx, id, body)
}

// Delete 删除
func (s *storageClassManager) Delete(ctx context.Context, id uint, name string) (err error) {
