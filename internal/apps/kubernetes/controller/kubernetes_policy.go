...
type PolicyController struct {
	policy service.PolicyManager // 使用服务接口
}

func NewPolicyController(policy service.PolicyManager) *PolicyController {
...
func (c *PolicyController) Create(ctx *gin.Context) {
	...
	if !success {
		ctx.JSON(http.StatusOK, gin.H{"success": false, "code": -1, "msg": "kubernetes policies already exist."})
		return
	}
