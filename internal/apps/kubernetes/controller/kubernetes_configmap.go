...
type ConfigmapController struct {
	configmap service.ConfigmapManager // 使用服务接口
}

// NewConfigmapController 创建新的 ConfigmapController 实例
func NewConfigmapController(configmap service.ConfigmapManager) *ConfigmapController {
...
func (c *ConfigmapController) Create(ctx *gin.Context) {
	...
	// 创建
	_, err := c.configmap.Create(ctx, id, ns, body)
	if err != nil {
		ctx.JSON(http.StatusOK, gin.H{"success": false, "code": -1, "msg": err.Error()})
		return
	}
...
func (c *ConfigmapController) Update(ctx *gin.Context) {
	...
	body.Name = name
	body.Namespace = ns
	_, err := c.configmap.Update(ctx, id, ns, body)
	if err != nil {
		ctx.JSON(http.StatusOK, gin.H{"success": false, "code": -1, "msg": err.Error()})
		return
	}
...
func (c *ConfigmapController) DeleteBatch(ctx *gin.Context) {
	idU, ok := ginhelper.RequireUintParam(ctx, "id")
	if !ok {
		return
	}
	...
	deleteBatchByNames(ctx, func(name string) error {
		return c.configmap.Delete(ctx, id, ns, name)
	})
}
