...
type GitlabRepository interface {
	...
	ListGroupProjects(token, baseURL string, groupID int64) ([]model.GitlabProject, error)
	ListProjectBranches(token, baseURL string, projectID int64) ([]model.GitlabRef, error)
	ListProjectTags(token, baseURL string, projectID int64) (data []model.GitlabRef, err error)
	GetRef(token, baseURL, ref string, projectID int64) (*model.GitlabRef, error)
	GetRepoURL(token, baseURL string, projectID int64) (string, error)
}
...
func (r *gitlabRepository) ListGroups(token, baseURL string) (data []model.GitlabGroup, err error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return nil, err
	}
	// 调用 GitLab API
...
func (r *gitlabRepository) ListGroupProjects(token, baseURL string, groupID int64) (data []model.GitlabProject, err error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return nil, err
...
func (r *gitlabRepository) ListProjectBranches(token, baseURL string, projectID int64) (data []model.GitlabRef, err error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return nil, err
...
func (r *gitlabRepository) ListProjectTags(token, baseURL string, projectID int64) (data []model.GitlabRef, err error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return nil, err
...
func (r *gitlabRepository) GetRef(token, baseURL, ref string, projectID int64) (data *model.GitlabRef, err error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return nil, err
	...
	return nil, fmt.Errorf("ref %s not found in project %d", ref, projectID)
}

func (r *gitlabRepository) GetRepoURL(token, baseURL string, projectID int64) (string, error) {
	// 创建 GitLab 客户端
	client, err := executor.GitlabClient(token, baseURL)
	if err != nil {
		return "", err
	...
	// 生成 repoURL
	repoURL := fmt.Sprintf("%s/%v.git", strings.TrimRight(baseURL, "/"), project.PathWithNamespace)
	return repoURL, nil
}
