...
type EnvironmentController struct {
	cred service.EnvironmentManager // 使用服务接口
}

func NewEnvironmentController(cred service.EnvironmentManager) *EnvironmentController {
	return &EnvironmentController{cred: cred}
}
...
func (c *EnvironmentController) List(ctx *gin.Context) {
	...
	params := pg.QueryParams{
		...
		Preloads:  []string{"CredHarbor", "CredArgoCD"},
		Filters:   map[string]interface{}{"project_id": projectID}, // 查询指定项目关联的 Env
	}
...
func (c *EnvironmentController) Get(ctx *gin.Context) {
	id, _ := strconv.Atoi(ctx.Param("envID"))
...
func (c *EnvironmentController) Update(ctx *gin.Context) {
	var data model.Environment
	var id, _ = strconv.Atoi(ctx.Param("envID")) // Env ID
...
func (c *EnvironmentController) Delete(ctx *gin.Context) {
	var id, _ = strconv.Atoi(ctx.Param("envID"))
